# apply_components_aggregator_patch_v2.ps1
# PowerShell installer (safe): adds product_v2 route file, http client, logger, gallery, audit rules, smoke CI, installs deps.
...
Write-Host "[DONE] Files created. Now add 2 lines into your Express server file (server.js/app.js):"
Write-Host "  ESM:"
Write-Host "    import productV2 from './src/server/routes/product_v2.js';"
Write-Host "    app.use('/api/product2', productV2);"
Write-Host "  CommonJS:"
Write-Host "    const productV2 = require('./src/server/routes/product_v2.js');"
Write-Host "    app.use('/api/product2', productV2);"
Write-Host ""
Write-Host "Then run: npm start  →  GET /api/product2?url=/product/EXAMPLE"
Write-Host "Logs: audit/logs/app.jsonl"
